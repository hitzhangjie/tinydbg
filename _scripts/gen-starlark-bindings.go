...
func genMapping(bindings []binding) []byte {
	...
	for _, binding := range bindings {
		...
		for i := range binding.argNames {
			...
			case "*github.com/hitzhangjie/tinydbg/service/api.LoadConfig":
			...
			case "*github.com/hitzhangjie/tinydbg/service/api.EvalScope":
...
func genDocs(bindings []binding) []byte {
	...
	for _, binding := range bindings {
		...
		fmt.Fprintf(&buf, "%s(%s) | Equivalent to API call [%s](https://pkg.go.dev/github.com/hitzhangjie/tinydbg/service/rpc2#RPCServer.%s)\n", binding.name, argNames, binding.fn.Name(), binding.fn.Name())
	}
