...
// getSuitableMethods returns the list of methods of service.RPCServer that are exported as API calls
...
func processServerMethods(serverMethods []*types.Func) []binding {
	...
	for i, fn := range serverMethods {
		...
		switch fn.Name() {
		case "Command":
			retType = "service.CommandOut"
		case "Restart":
			retType = "service.RestartOut"
		case "State":
			retType = "service.StateOut"
		}
...
func genMapping(bindings []binding) []byte {
	...
	for _, binding := range bindings {
		...
		for i := range binding.argNames {
			...
			case "*github.com/hitzhangjie/dlv/service/api.LoadConfig":
			...
			case "*github.com/hitzhangjie/dlv/service/api.EvalScope":
...
func genDocs(bindings []binding) []byte {
	...
	for _, binding := range bindings {
		...
		fmt.Fprintf(&buf, "%s(%s) | Equivalent to API call [%s](https://godoc.org/github.com/hitzhangjie/dlv/service#RPCServer.%s)\n", binding.name, argNames, binding.fn.Name(), binding.fn.Name())
	}
