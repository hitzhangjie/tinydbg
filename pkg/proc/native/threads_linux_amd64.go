...
func (t *nativeThread) withDebugRegisters(f func(*amd64util.DebugRegisters) error) error {
	...
	t.dbp.execPtraceFunc(func() {
		...
		for i := range debugregs {
			...
			_, _, err = sys.Syscall6(sys.SYS_PTRACE, sys.PTRACE_PEEKUSR,
				uintptr(t.ID),
		...
		drs := amd64util.NewDebugRegisters(&debugregs[0],
			&debugregs[1], &debugregs[2], &debugregs[3], &debugregs[6], &debugregs[7])
		...
		if drs.Dirty {
			for i := range debugregs {
				...
				_, _, err = sys.Syscall6(sys.SYS_PTRACE, sys.PTRACE_POKEUSR,
					uintptr(t.ID),
					uintptr(debugRegUserOffset+uintptr(i)*unsafe.Sizeof(debugregs[0])),
					uintptr(debugregs[i]),
					0,
